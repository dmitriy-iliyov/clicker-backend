...
public class WalletsServiceImpl implements WalletsService {
    ...
    public WalletResponseDto update(WalletUpdateDto walletUpdateDto) {
        WalletEntity walletEntity = walletsRepository.findWithCurrencyById(walletUpdateDto.getId()).orElseThrow(
                WalletNotFoundException::new
        );
    ...
    public void updateUserWallets(UUID userId, Set<WalletUpdateDto> walletUpdateDtos) {
        ...
        Set<WalletEntity> updatedWallets = walletUpdateDtos.stream()
                .map(walletUpdateDto -> {
                    WalletEntity walletEntity = existedWallets.get(walletUpdateDto.getId());
                    walletMapper.updateEntityFromDto(walletUpdateDto, walletEntity, currencyService);
                    return walletEntity;
                })
